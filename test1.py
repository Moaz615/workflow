...
LANGUAGES = {
    ...
    'es': {
        ...
        'ffmpeg_missing': '¡Falta ffmpeg! Las descargas de audio no funcionarán. Por favor, coloque ffmpeg in la carpeta de la aplicación o instale ffmpeg y agréguelo a su PATH.',
    ...
    'fr': {
        ...
        'ffmpeg_missing': 'ffmpeg est manquant ! Les téléchargements audio ne fonctionneront pas. Veuillez placer ffmpeg dans le dossier de l\'application ou installer ffmpeg et l\'ajouter à votre PATH.',
    ...
    'ar': {
        ...
        'ffmpeg_missing': 'ffmpeg غير موجود! لن تعمل تنزيلات الصوت. يرجى وضع ffmpeg في مجلد التطبيق أو تثبيت ffmpeg وإضافته إلى مسار النظام.',
    ...
    'ja': {
        ...
        'ffmpeg_missing': 'ffmpegが見つかりません！音声のダウンロードは機能しません。ffmpegをアプリのフォルダに配置するか、ffmpegをインストールしてPATHに追加してください。',
    ...
    def __init__(self, settings: Settings, downloader: VideoDownloader):
        ...
        try:
            if sys.platform.startswith("win"):
                icon_path = resource_path('moaz.ico')
                if os.path.exists(icon_path):
                    self.root.iconbitmap(icon_path)
            else:
                # For other platforms, PhotoImage can handle PNG.
                # It might handle ICO on some Linux WMs, so we try that as a fallback.
                icon_path_png = resource_path('moaz.png')
                if os.path.exists(icon_path_png):
                    img = tk.PhotoImage(file=icon_path_png)
                    self.root.tk.call('wm', 'iconphoto', self.root._w, img)
                else:
                    icon_path_ico = resource_path('moaz.ico')
                    if os.path.exists(icon_path_ico):
                        img = tk.PhotoImage(file=icon_path_ico)
                        self.root.tk.call('wm', 'iconphoto', self.root._w, img)
        except Exception as e:
            self.log_message(f"Could not set application icon: {e}")
